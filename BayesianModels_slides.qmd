...
mu0 = 0
sigma0 = 1
y0 = make_y(mu0, sigma0)
fig.add_trace(go.Scatter(x=x, y=y0, mode='lines', name='Gaussian'))

# Add sliders
mu_vals = np.round(np.linspace(-5, 5, 11), 1)
sigma_vals = np.round(np.linspace(0.5, 3, 6), 2)
...
- <img src="/images/certainty_belief.jpeg" width="800px">
...
# Define value ranges
mu_vals = np.round(np.linspace(-10, 10, 41), 1)       # μ from -10 to 10, step 0.5
sigma_vals = np.round(np.linspace(0.1, 5.0, 50), 2)   # σ from 0.1 to 5.0, step 0.1

# Initial values
mu0 = 0.0
sigma0 = 1.0
y0 = gaussian_pdf(mu0, sigma0)

# Create figure
fig = go.Figure()
fig.add_trace(go.Scatter(x=x, y=y0, mode='lines', name='Gaussian'))

# Create slider steps (combined μ and σ values)
steps = []
for mu in mu_vals:
    for sigma in sigma_vals:
        y = gaussian_pdf(mu, sigma)
        steps.append(dict(
            method="update",
            label=f"μ={mu}, σ={sigma}",
            args=[{"y": [y]},
                  {"title": f"Gaussian Distribution — N({mu}, {sigma}²)"}]
        ))
...
fig.update_layout(
    sliders=[{
        "active": 0,
        "steps": steps,
        "currentvalue": {"prefix": "μ and σ: "}
    }],
    xaxis_title="θ",
    yaxis_title="Probability Density",
    title=f"Gaussian Distribution — N({mu0}, {sigma0}²)",
    showlegend=False
)
...
- <img src="/images/interactive_gaussian.png" width="500px">

## Interpreting probability distributions
- See "interpreting probability distributions"
- 📂 `notebooks/gaussian_interactive_plot.ipynb`
- <img src="/images/interpreting_distributions.png" width="600px">
