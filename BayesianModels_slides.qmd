...
## Back to cogntive task: Observed vs. latent
- **Observed**: number correct ($k/n$)
- **Latent**: ability ($\theta$)
- Same $\theta$ → different $k/n$
- Different $\theta$ → same $k/n$ 
- Bayesian models allow us to infer latent causes of observed data

## Beliefs as distributions
- Probability distributions encode beliefs
- Shape reflects belief; spread reflects uncertainty
- e.g., confident high, uncertain low, etc.

## Certainty of beliefs
- <img src="/images/certainty_belief.jpeg" width="1000px">
...
## test
<iframe src="/notebooks/beta_interactive_plot.html" width="100%" height="600px" style="border:none;"></iframe>


## Interactive Beta Distribution (Plotly)
...
def plot_beta_plotly(k=1, n=1):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=y, fill='tozeroy', name=f'Beta({alpha}, {beta_param})'))
    fig.update_layout(
...
## Prior over $\theta$
- Lets start with our prior beliefs for $\theta$
- Suppose we believe that all abilities are equally likely. 
- What would this look like? 
- Uniform prior: equal belief over (0,1)
- Represents maximum uncertainty

## Updating beliefs according to evidence
- Data: $k = 9$ correct of $n = 10$
...
- Ensures it is a probability distribution
- Independent of $\theta$
...
- Posterior quantifies belief about $\theta$
- If 50% is above 0.65 → $p(\theta > 0.65) = 0.5$
