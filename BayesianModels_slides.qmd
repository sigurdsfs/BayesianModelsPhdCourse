...
format:  # ← Specifies output formats. You can define multiple formats like revealjs (slides), html, pdf, etc.
  revealjs:  # ← This block defines settings specifically for Reveal.js slide output.
    ...
    footer:   |  # ← Adds a custom HTML footer across all slides. The pipe (|) means the following lines are literal block text.
        <a href="https://tinyurl.com/bayesBinder" target="_blank">
        tinyurl.com/bayesBinder </a>&nbsp;&nbsp;&nbsp;  
...
## Schedule
- Up to date schedule 
- <iframe src="https://tinyurl.com/bayesSchedule"  width="1000px" height="500px" style="border:1px solid #ccc; border-radius:10px;"></iframe>
- link in footer 👇   
...
- Latent vs. observable variables
- Beliefs and evidence  
- Estimation methods
- Why Bayes?

## What is Bayesian modeling, really?
- Bayesian modeling is just probability theory in action.
- Probability theory is logic applied to uncertainty.
- So Bayesian modeling = **rational belief updating**.

## What are we really doing?
- We're not doing "Bayesianism" — we're using probability to reason about what we can't see.
- We build models that update beliefs in light of evidence — just like minds do.

## Bayesianism in quotes
- *"Probability theory is nothing but common sense reduced to calculation."* — Laplace
- *"The rules of probability are the rules of consistent reasoning."* — E.T. Jaynes

## Bayesianism in a nutshell
- Bayes is probability. Probability is logic.
- Logic is consistency. Consistency is rationality.
- So this course is about something sacred:
  **thinking clearly about minds, brains, and behavior.**

## General principles
- Belief → Evidence → Updated belief
- <span class="grey">In other words</span> 
- Prior → Evidence → Posterior

## Example: a cognitive task
- 10 trials of equal difficulty, binary outcomes (correct or incorrect) 
- <span class="grey">e.g., go-no-go, task-switching, stop-go... </span>
- <img src="/images/gonogo.png" width="800px">

## Cognitive ability
- We want to estimate ability $\theta$ from behavior
- Ability here is: 
  - ~ *tendency for correct performance*
  - *latent* — we can’t observe it directly
  - theoretical
- Behavior here is:
  - observable data
  - number of correct trials 
  - <span class="grey">e.g. k correct out of n trials - 8/10</span>

## Latent vs. observable cartoon
- <img src="/images/latent_observable.png" width="1000px">

## Latent variables
- Latent variables are **central** to cognitive modeling
- They let us **explain**, not just describe
- **Descriptive**: “8/10 correct”
- **Explanatory**: “What ability produced that behavior?”
- Psychology / neuroscience is always about **hidden causes**

## What’s observed vs. what’s hidden
- **Observable**: number correct - <span class="grey">e.g. k / n </span>
- **Hidden/latent**: ability ($\theta$) 
- Same $\theta$ → different observable 
- Same performance ≠ same ability
- Bayesian models infer latent causes of observable data

## Why do we model hidden causes?
- Our theories live in the **hidden**
- We care about *why*, not just *what*
- Real questions ask about causes, not outcomes
- e.g., *Do the two groups truly differ in their ability?*
- So we model what we **can’t observe directly**

## Prior belief over $\theta$
- If agnostic, use **uniform prior**
- Equal probability for all $\theta$ in (0,1)
- Maximum uncertainty, no bias
- 

## Representing beliefs as distributions
- Probabilities represent belief
- Shape = belief; spread = uncertainty
- Use sliders to explore different priors
- e.g., certain low, certain high, uncertain

## Update prior with evidence
- Data: 9/10 correct
- Prior $\rightarrow$ Likelihood $\rightarrow$ Posterior
- $p(\theta) \rightarrow p(D|\theta) \rightarrow p(\theta|D)$
...
- It normalizes the posterior (ensures it sums to 1)
...
- Interpreted as belief about $\theta$
- If 50% is above 0.65, then $p(\theta > 0.65) = 0.5$
- Simple, intuitive interpretation

## Bayesian credibility intervals
- $\text{BCI}_{95\%} [0.4, 0.8]$ means 95% chance $\theta$ is in that range
- This is what we *want* classical confidence intervals to mean

## Sequential updating (part 1)
- Get new data: 3/5 additional trials correct
- Previously: 9/10 correct → now 12/15
- Combine new data with old

## Sequential updating (part 2)
- Update in two steps or one — same result
- Prior $\rightarrow$ Posterior $\rightarrow$ Posterior again
- Or Prior $\rightarrow$ All data $\rightarrow$ Posterior

## Analytic solution
- Beta prior: $\text{Beta}(1,1)$ (uniform)
- Posterior: $\text{Beta}(1+k,1+n-k)$
- Where $k$ = correct, $n$ = total trials

## Interactive beta distribution
- Adjust $k$ and $n$ to see posteriors
- When is the posterior most uncertain?
- When does it strongly prefer high or low $\theta$?

## Conjugate priors
- Prior & posterior in same family → conjugate
- Nice when possible
...
- Works even when no analytic solution exists
- Feels a bit magical, but it’s robust

## Analytic vs. Sampling
- Plot analytic posterior vs. MCMC samples
- Both aim to estimate same thing
- Useful to compare

## How MCMC works
- Two paths forward:
  - Red pill: Learn how it works (see video)
  - Blue pill: Accept it works, move on 😄

## Why Bayesian modeling?
- **Flexible**: Handles complex, missing, or messy data
- **Principled**: Always quantifies uncertainty
- **Intuitive**: Matches human reasoning
- **Accessible**: Simple to use once understood
<!-- #endregion -->



<!-- #region Modelling a binary process-->
...
- One heuristic is the R-hat
\[
\hat{R} = \frac{\text{var}(	ext{within-chain})}{\text{var}(	ext{across-chain})}
\]
-Rule of thumb: $\hat{R}$ should be 1-1.01 for convergence to be achieved
 
footer
Learn more: Brooks & Gelman (1998).  
<!-- #endregion -->
